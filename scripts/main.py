...
LastEditTime: 2021-12-21 17:43:37
'''
...
def init_project(cfg):
    ...
    if cfg.TRAIN.if_cuda:
        ...
        torch.backends.cudnn.benchmark = True       # used for selecting a optimal convolutional algorithm
        # torch.backends.cudnn.deterministic = True # used for fixing the convolutional algorithm
    ...
    while iters <= cfg.TRAIN.total_iters:
        ...
        if cfg.TRAIN.if_valid:
            if iters % cfg.TRAIN.valid_freq == 0 or iters == 1:
                ...
                mean_dice, mean_jac = target_evaluation(preds)
                logging.info('model-%d, dice=%.6f, jac=%.6f' % (iters, mean_dice, mean_jac))
                writer.add_scalar('valid/dice', mean_dice, iters)
                writer.add_scalar('valid/jac', mean_jac, iters)
