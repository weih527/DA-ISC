...
if __name__ == "__main__":
    ...
    input = np.random.random((2,3,512,512)).astype(np.float32)
    x = torch.tensor(input).to('cuda:0')

    model = get_accel_deeplab_v2(num_classes=2, multi_level=True).cuda()
    cf_aux, cf, kf_aux, kf = model(x, x)
    print(cf_aux.shape)
    print(cf.shape)
    print(kf_aux.shape)
    print(kf.shape)
