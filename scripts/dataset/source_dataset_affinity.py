...
from utils.affinity import gen_affs
...
class sourceDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        current_label = (current_label / 255).astype(np.float32)
        aux_label = (aux_label / 255).astype(np.float32)
        affs = gen_affs(current_label, aux_label, shifts=[0,1,3,5,9])
        ...
        affs = torch.from_numpy(affs.astype(np.float32)).long()

        return current_img, current_label, aux_img, aux_label, affs
